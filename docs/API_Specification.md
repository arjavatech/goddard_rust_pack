...
Role Permissions:
- SuperAdmin: System-wide access, school management
- Admin: School-wide access, user invitations, enrollment approval
- Teacher: Classroom access, student progress
- Parent: Own children only, enrollment forms
```
...
Response (201):
{
  ...
  "invite_link": "https://app.goddard.com/signup?invite_id=uuid"
}
```
...
INSERT INTO user_invitations (id, school_id, email, role, invite_id, expires_at, created_by)
VALUES (gen_random_uuid(), $1, $2, $3, gen_random_uuid(), NOW() + INTERVAL '7 days', $4);
...
2. Verify invite_id exists and not expired
3. Match JWT email with invitation email
4. Check invitation not already used
5. Link user to invitation record
...
- 422: Email mismatch with JWT
```

**Database Operations:**
```sql
-- Validate invitation with JWT email
SELECT id, school_id, email, role, is_used, expires_at
FROM user_invitations
WHERE invite_id = $1 AND email = $2 AND expires_at > NOW() AND is_used = false;

-- Update existing user record (user already exists from Supabase Auth)
UPDATE users
SET invite_id = $1, is_verified = true, verified_at = NOW()
WHERE email = $2 AND school_id = $3;
...
3. Query users table for role and school_id
4. Return minimal user context
```

**Database Query:**
```sql
SELECT role, school_id FROM users WHERE id = $1 AND is_verified = true;
```
