...
use crate::{
    ...
    models::employee::{
        Employee, EmployeeWithUser, EmployeeInviteRequest, EmployeeInviteResponse,
        UpdateEmployeeRequest, EmployeeFormTemplate, CreateEmployeeFormTemplateRequest,
...
impl EmployeeService {
    ...
    pub async fn invite_employee(&self, req: EmployeeInviteRequest) -> ApiResult<EmployeeInviteResponse> {
        // Validate email basic format
        if req.email.trim().is_empty() || !req.email.contains('@') {
            return Err(AppError::Validation("Invalid email address".to_string()));
        }

        let school_name = self.school_dao.get_school_name(&req.school_id).await
            ...
            .await
        {
            ...
            Err(AppError::Conflict(_)) => {
                ...
                if self.employee_dao
                    .get_employee_by_email_and_school(&req.email, req.school_id)
                    .await?
                    .is_some()
                {
                    return Err(AppError::Conflict(
                        format!("Employee with email {} already exists in this school", req.email)
                    ));
                }
        ...
        let employee = self.employee_dao.create_employee(
            user_id,
            req.school_id,
            req.phone.as_deref(),
        ...
        let invite_link = format!("{}/enrollments/activate/{}", Self::api_base_url(), invite_token);
    ...
    pub async fn get_current_employee(&self, user_id: Uuid, school_id: Uuid) -> ApiResult<EmployeeWithUser> {
        self.employee_dao.get_employee_by_user_id_with_user(user_id, school_id).await?
            .ok_or_else(|| AppError::NotFound("Employee record not found for current user".to_string()))
    }
    ...
    pub async fn get_employee_by_id(&self, employee_id: Uuid, school_id: Uuid) -> ApiResult<EmployeeWithUser> {
        self.employee_dao.get_employee_by_id(employee_id, school_id).await?
            .ok_or_else(|| AppError::NotFound("Employee not found".to_string()))
    }
    ...
    pub async fn create_form_template(&self, req: CreateEmployeeFormTemplateRequest) -> ApiResult<EmployeeFormTemplate> {
        self.employee_form_template_dao.create_template(&req).await
    }

    pub async fn get_form_templates(&self, school_id: Uuid) -> ApiResult<Vec<EmployeeFormTemplate>> {
        self.employee_form_template_dao.get_templates_by_school(school_id).await
    }

    pub async fn update_form_template(&self, req: UpdateEmployeeFormTemplateRequest) -> ApiResult<EmployeeFormTemplate> {
    ...
    pub async fn assign_form(&self, req: AssignEmployeeFormRequest, assigned_by: Uuid) -> ApiResult<EmployeeFormAssignment> {
        // Get employee to find user_id and email
        let employee = self.employee_dao.get_employee_by_id(req.employee_id, req.school_id).await?
            .ok_or_else(|| AppError::NotFound("Employee not found".to_string()))?;
        ...
        let assignment = self.employee_form_assignment_dao.create_assignment(
            req.employee_id,
            employee.user_id,
            req.school_id,
        ...
        let due_date_str = template.due_date
        ...
        tokio::spawn(async move {
            if let Err(e) = email_svc.send_employee_form_assigned_email(
                &email, &employee_name, &form_name, &due_date_str, &dashboard_url,
            ).await {
    ...
    pub async fn get_assignments_by_employee(&self, employee_id: Uuid) -> ApiResult<Vec<EmployeeFormAssignmentWithTemplate>> {
        self.employee_form_assignment_dao.get_assignments_by_employee(employee_id).await
    }
    ...
    pub async fn review_assignment(&self, req: ReviewEmployeeFormRequest, reviewer_id: Uuid) -> ApiResult<EmployeeFormAssignment> {
        let assignment = self.employee_form_assignment_dao.review_assignment(
            req.assignment_id,
            req.school_id,
            &req.status,
            reviewer_id,
        ...
        let employee = self.employee_dao.get_employee_by_id(assignment.employee_id, req.school_id).await;
        let template = self.employee_form_template_dao
            .get_template_by_id(assignment.employee_form_template_id, req.school_id).await;
        ...
        if let (Ok(Some(emp)), Ok(Some(tpl))) = (employee, template) {
            ...
            tokio::spawn(async move {
                let result = if status == "approved" {
                    email_svc.send_employee_form_approved_email(&email, &name, &form_name, &notes).await
                } else {
                    email_svc.send_employee_form_rejected_email(&email, &name, &form_name, &notes).await
                };
                if let Err(e) = result {
    ...
    pub async fn delete_assignment(&self, assignment_id: Uuid, school_id: Uuid) -> ApiResult<()> {
        self.employee_form_assignment_dao.delete_assignment(assignment_id, school_id).await
    }
    ...
    pub async fn handle_form_webhook(
        ...
        let submission = self.employee_form_submission_dao.upsert_submission(
            school_id,
            employee_id,
            assignment_id,
            template_id,
    ...
    pub async fn get_submission_by_assignment(&self, assignment_id: Uuid) -> ApiResult<Option<EmployeeFormSubmission>> {
        self.employee_form_submission_dao.get_submission_by_assignment(assignment_id).await
    }

    pub async fn get_submissions_by_employee(&self, employee_id: Uuid) -> ApiResult<Vec<EmployeeFormSubmission>> {
        self.employee_form_submission_dao.get_submissions_by_employee(employee_id).await
    }

    // ─── Bulk Reminders ─────────────────────────────────────────────────────────

    pub async fn send_bulk_reminders(&self, req: BulkEmployeeFormReminderRequest) -> ApiResult<BulkEmployeeReminderResponse> {
        let dashboard_url = Self::employee_dashboard_url();
        ...
        for reminder in &req.reminders {
            let result = self.email_service.send_employee_form_reminder_email(
            ...
            match result {
                Ok(_) => total_sent += 1,
                Err(e) => {
                    tracing::warn!("Reminder email failed for {}: {}", reminder.employee_email, e);
                    total_failed += 1;
