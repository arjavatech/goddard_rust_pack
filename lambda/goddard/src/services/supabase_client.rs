...
pub struct UserMetadata {
    #[serde(
        serialize_with = "serialize_uuid_option",
        deserialize_with = "deserialize_uuid_option"
    )]
...
impl UserMetadata {
    ...
    /// Claims consumed by TapTime must live in Supabase `app_metadata`, because
    /// a signed-in user can edit `user_metadata`.  Only school staff receive
    /// these claims; parents keep the normal Supabase provider metadata only.
    pub fn to_taptime_app_metadata(&self) -> serde_json::Value {
        let taptime_role = match self.role.as_deref() {
            Some("Employee") => Some("employee"),
            Some("Admin") => Some("admin"),
            Some("SuperAdmin") => Some("super_admin"),
            _ => None,
        };
        ...
        if let (Some(school_id), Some(role)) = (self.school_id, taptime_role) {
            metadata["school_id"] = json!(school_id.to_string());
            metadata["taptime_role"] = json!(role);
        }
        metadata
    }
}
...
impl SupabaseClient {
    ...
    pub async fn resend_invitation(&self, email: &str) -> Result<(), AppError> {
        ...
        let invite_response = self
            ...
            .await
            .map_err(|e| {
                AppError::ExternalService(format!("Failed to send {}: {}", email_type, e))
            })?;
    ...
    /// Server-owned TapTime claims.  These are written only through the
    /// Supabase Admin API so a browser can never elevate its TapTime role or
    /// choose a different school.
    pub async fn set_taptime_access_claims(
        &self,
        user_id: Uuid,
        school_id: Uuid,
        role: &str,
    ) -> Result<(), AppError> {
        let taptime_role = match role {
            "Employee" => "employee",
            "Admin" => "admin",
            "SuperAdmin" => "super_admin",
            _ => return Err(AppError::Validation("Unsupported TapTime access role".into())),
        };
        ...
        if !response.status().is_success() {
            return Err(AppError::ExternalService(format!(
                "Failed to load Supabase user for TapTime access: {}", response.status())));
        }
        let user: serde_json::Value = response.json().await
            .map_err(|e| AppError::ExternalService(format!("Invalid Supabase user response: {e}")))?;
        let mut metadata = user.get("app_metadata").and_then(|v| v.as_object()).cloned().unwrap_or_default();
        metadata.insert("school_id".into(), json!(school_id.to_string()));
        metadata.insert("taptime_role".into(), json!(taptime_role));
        let response = self.client
            ...
            .send().await
            .map_err(|e| AppError::ExternalService(format!("Failed to update TapTime claims: {e}")))?;
        if !response.status().is_success() {
            return Err(AppError::ExternalService(format!(
                "Failed to update TapTime claims: {}", response.status())));
        }
        Ok(())
    }
    ...
    pub async fn create_user_with_signup_confirmation(
        ...
        let create_user_body = json!({
            ...
            "app_metadata": metadata.to_taptime_app_metadata()
        });

        tracing::debug!(
            "Creating user with body: {}",
        ...
        let user_data: serde_json::Value = create_response.json().await.map_err(|e| {
            AppError::ExternalService(format!("Failed to parse user creation response: {}", e))
        })?;
        ...
        let user_id = user_data
            ...
            .ok_or_else(|| {
                AppError::ExternalService("User ID not found in response".to_string())
            })?;
    ...
    pub async fn create_user_invitation_enhanced(
        &self,
        email: &str,
        metadata: UserMetadata,
        ...
        tracing::info!(
            "[SupabaseClient] Sending email to {} with template type: {}",
            email,
            template_type
        );
        ...
        tracing::info!(
            "📋 [SupabaseClient] Transformed metadata JSON: {}",
        ...
        if let Some(school_name) = user_metadata.get("school_name") {
            tracing::info!(
                "📋 [SupabaseClient] school_name in metadata: {:?}",
                school_name
            );
        } else {
        ...
        self.create_user_invitation_with_template(email, Some(user_metadata), template_type)
            .await
    }

    pub async fn create_user_invitation_with_template(
        &self,
        email: &str,
        ...
        let invite_data: serde_json::Value = invite_response.json().await.map_err(|e| {
            AppError::ExternalService(format!("Failed to parse invite response: {}", e))
        })?;
        ...
        let user_id = invite_data
            ...
            .ok_or_else(|| {
                AppError::ExternalService(format!(
                    "User ID not found in invite response. Response: {}",
    ...
    pub async fn create_user_invitation(
        ...
        self.create_user_invitation_with_template(email, user_metadata, "signup")
            .await
    }
    ...
    pub async fn create_auth_user(&self, email: &str) -> Result<uuid::Uuid, AppError> {
        ...
        let auth_user_id = uuid::Uuid::parse_str(&user_id_string).map_err(|e| {
            AppError::ExternalService(format!("Invalid user ID format from Supabase: {}", e))
        })?;
    ...
    pub async fn get_user_email_by_id(&self, user_id: uuid::Uuid) -> Result<String, AppError> {
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                AppError::ExternalService(format!("Failed to get user from Supabase: {}", e))
            })?;
        ...
        if !response.status().is_success() {
            ...
            if status_code == 404 {
                return Err(AppError::NotFound(
                    "User not found in Supabase auth".to_string(),
                ));
            }
        ...
        let user_data: serde_json::Value = response.json().await.map_err(|e| {
            AppError::ExternalService(format!("Failed to parse user response: {}", e))
        })?;
        ...
        let email = user_data
            ...
            .ok_or_else(|| {
                AppError::ExternalService("Email not found in user response".to_string())
            })?;
    ...
    pub async fn get_user_auth_details(
        &self,
        user_id: uuid::Uuid,
    ) -> Result<(String, DateTime<Utc>, bool), AppError> {
        let user_id_str = user_id.to_string();
        ...
        let created_at_str = user_data
            ...
            .ok_or_else(|| {
                AppError::ExternalService("Created_at not found in user response".to_string())
            })?;
    ...
    pub async fn clear_auth_table(&self) -> Result<(), AppError> {
        ...
        let user_ids: Vec<String> =
            if let Some(users_array) = users.get("users").and_then(|v| v.as_array()) {
                users_array
                    .iter()
        ...
        for user_id in user_ids {
            let delete_response = self
                ...
                .await
                .map_err(|e| {
                    AppError::ExternalService(format!("Failed to delete user {}: {}", user_id, e))
                })?;
    ...
    pub async fn verify_jwt_and_get_user(
        &self,
        jwt_token: &str,
    ...
    pub async fn send_password_reset_email(&self, email: &str) -> Result<(), AppError> {
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                AppError::ExternalService(format!("Failed to send password reset email: {}", e))
            })?;
    ...
    pub async fn generate_signup_link(&self, email: &str) -> Result<String, AppError> {
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                AppError::ExternalService(format!("Failed to call generate_link: {}", e))
            })?;
        ...
        let data: serde_json::Value = response.json().await.map_err(|e| {
            AppError::ExternalService(format!("Failed to parse generate_link response: {}", e))
        })?;
        ...
        let action_link = data
            ...
            .ok_or_else(|| {
                AppError::ExternalService(
                    "action_link not found in generate_link response".to_string(),
                )
            })?;
    ...
    pub async fn send_parent_invite_email(
        ...
        let confirmation_url = format!(
            ...
            self.api_base_url, invite_token
        );
        let html_body =
            super::email_templates::parent_invite_html(first_name, last_name, &confirmation_url);
        match self
            .email_service
            .dispatch(
                email,
                "Invitation to Create an Account for The Goddard School Admission",
                &html_body,
            )
            .await
        {
            Ok(_) => {
                tracing::info!(
                    "✅ Parent invite email sent to {} (token: {})",
                    email,
                    invite_token
                );
                Ok(true)
    ...
    pub async fn send_admin_invite_email(
        ...
        match self
            .email_service
            .dispatch(email, &subject, &html_body)
            .await
        {
            Ok(_) => {
                tracing::info!(
                    "✅ Admin invite email sent to {} (token: {})",
                    email,
                    invite_token
                );
                Ok(true)
    ...
    pub async fn create_user_with_password_in_supabase(
        ...
        let create_response = self
            ...
            .await
            .map_err(|e| {
                AppError::ExternalService(format!("Failed to create user with password: {}", e))
            })?;
