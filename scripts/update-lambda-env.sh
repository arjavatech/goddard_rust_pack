...
# Deploy Environment Variables to AWS Lambda (Fixed Version)
...
echo "$ENV_JSON" | jq -r '.Variables | to_entries | .[0:5] | .[] | "  \(.key) = \(.value)"' 2>/dev/null || {
...
if AWS_PROFILE="$AWS_PROFILE" aws lambda update-function-configuration \
    --region "$AWS_REGION" \
